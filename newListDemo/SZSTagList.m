...
{
    Class tagClass = _tagClass?_tagClass : [SZSTagButton class];
    
    // 创建标签按钮
    SZSTagButton *tagButton = [tagClass buttonWithType:UIButtonTypeCustom];
    if (_tagClass == nil) {
...
{
    // 获取对应的标题按钮
    SZSTagButton *button = self.tags[tagStr];
...
- (void)updateTagButtonFrame:(NSInteger)i extreMargin:(BOOL)extreMargin
{
    ...
    // 获取当前按钮
    SZSTagButton *tagButton = self.tagButtons[i];
    // 判断是否设置标签的尺寸
